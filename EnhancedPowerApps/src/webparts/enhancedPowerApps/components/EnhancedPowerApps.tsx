...
export default class EnhancedPowerApps extends React.Component<IEnhancedPowerAppsProps, IEnhancedPowerAppsState> {
  ...
  public render(): React.ReactElement<IEnhancedPowerAppsProps> {
    ...
    const staticPropUrlParam: string = useStaticProp && staticPropName !== null && staticPropValue !== null ? `&${encodeURIComponent(staticPropName)}=${encodeURIComponent(staticPropValue)}`:'';

    // Build the frame url
    let frameUrl: string = `${appUrl}?source=SPClient-EnhancedPowerAppsWebPart&amp;locale=${locale}&amp;enableOnBehalfOf=true&amp;authMode=onbehalfof&amp;hideNavBar=true&amp;`;
    frameUrl += `${dynamicPropValue}`;
    frameUrl += `${staticPropUrlParam}`;
    frameUrl += `${themeParams}`;
    frameUrl += `&locale=${locale}`;
