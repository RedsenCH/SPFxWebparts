...
> {
  ...
  constructor(props: ITableOfContentsProps) {
    ...
    this.toggleShowHideFloatingMenu =
      this.toggleShowHideFloatingMenu.bind(this);
  }
  ...
  */
  // Déclenche une action une fois que le DOM de la page est disponible
  public componentDidMount(): void {
    // Cibler l'élément du DOM que tu veux observer
    const targetNode = document.querySelector(".CanvasComponent"); // Remplacer par ton sélecteur spécifique
    if (targetNode) {
      // Créer le MutationObserver
      this.observer = new MutationObserver((mutationsList) => {
        let headerAdded = false; // Variable pour détecter l'ajout d'en-têtes
        mutationsList.forEach((mutation) => {
          if (mutation.type === "childList") {
            // Vérifie les ajouts d'éléments
            ...
            if (headerAdded) {
              // Une nouvelle balise a été ajoutée. On met à jour l'état pour trigger un re-render.
              this.setState({}); // Ceci entraîne un re-render
            }
            // Vérifie les suppressions d'éléments
            ...
            if (headerAdded) {
              // Si un élément a été supprimé, également mettre à jour l'état pour forcer le re-render
              this.setState({}); // Ceci entraîne un re-render
            }
          }
        });
      ...
      // Configurer les options que tu veux surveiller
      const config = {
  ...
  // On peut aussi détacher l’observateur dans componentWillUnmount pour éviter les fuites mémoire
  ...
  public render(): React.ReactElement<ITableOfContentsProps> {
    ...
    const headers = this.getHtmlElements(querySelector)
      .filter(this.filterEmpty)
      .filter(this.filterAside);
    ...
    const hideInMobileViewClass = this.props.hideInMobileView
      ? styles.hideInMobileView
      : "";
    ...
    const floatingButtonStyles: IButtonStyles = {
      root: {
        ...
        width: 40,
        height: 40,
        backgroundColor: this.props.floatingMenuButtonBackgroundColor
          ? this.props.floatingMenuButtonBackgroundColor
          : this.props.theme?.palette?.themePrimary,
      ...
      icon: {
        fontSize: 20,
        color: this.props.floatingMenuButtonIconColor
          ? this.props.floatingMenuButtonIconColor
    ...
    const floatingButtonIcon: IIconProps = {
      iconName: this.props.floatingMenuButtonIcon
        ? this.props.floatingMenuButtonIcon
        : "BulletedList2",
    };

    return (
      <>
        {this.props.floatingMenu &&
            ...
            <div className={styles.floatingTableOfContent}>
              ...
              >
                <section className={styles.tableOfContents}>
                  ...
                  />
                  <nav className="CustomSPToc">{toc}</nav>
                </section>
              </Stack>
              <IconButton
        ...
        return (
          <li key={index} className={`toc_level_${currentLevel}`}>
            ...
            {link.childNodes.length > 0 ? (
              <ul>{this.renderLinks(link.childNodes, nestedLevel)}</ul>
            ) : (
              ""
            )}
          </li>
